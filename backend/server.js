// ============================================
// ArcDrop Backend - CORRECTED for Circle Developer Controlled Wallets
// idempotencyKey goes in BODY, not headers
// ============================================
...
// ============================================
// HELPER: Generate Idempotency Key (UUID v4)
// ============================================
    ...
    if (!user) {
      ...
      if (!process.env.CIRCLE_WALLET_SET_ID) {
        return res.status(500).json({
          error: 'Server misconfigured: CIRCLE_WALLET_SET_ID missing',
          fix: 'Add CIRCLE_WALLET_SET_ID to Render Environment Variables'
        });
      }

      if (!process.env.CIRCLE_ENTITY_SECRET_CIPHERTEXT) {
        return res.status(500).json({
          error: 'Server misconfigured: CIRCLE_ENTITY_SECRET_CIPHERTEXT missing',
          fix: 'Generate entity secret ciphertext in Circle Console and add to Render'
        });
      }

      // Create wallet via Circle Developer Controlled Wallets
      // FIX: idempotencyKey goes in the BODY, not headers
      const walletRes = await axios.post(
        `${CIRCLE_API}/developer/wallets`,
        {
          idempotencyKey: generateIdempotencyKey(),  // ✅ IN BODY
...
app.get('/api/wallet/:walletId/balance', async (req, res) => {
  try {
    ...
    // Get USDC balance on-chain via RPC
...
app.post('/api/tip/send', async (req, res) => {
  try {
    ...
    // Execute gasless transfer via Circle Developer Controlled Wallets API
    // FIX: idempotencyKey goes in the BODY
